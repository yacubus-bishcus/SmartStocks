...
class OutputHandler:
    ...
    def make_dates_timezone_unaware(self, df):
        ...
        # Iterate over each column to remove timezone info if present
    ...
    def write_to_excel(self, df, sheet_name, string_index=False):
        if string_index:
        ...
        if os.path.exists(filepath):
            with pd.ExcelWriter(filepath, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
                ...
                logger.warning(f"Replacing data in {filepath}")
